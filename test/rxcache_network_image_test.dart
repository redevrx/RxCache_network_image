...
@GenerateNiceMocks(
  [MockSpec<BaseRxCacheManager>(), MockSpec<File>()],
)
void main() {
  ...
  group('test cache manager', () {
    ...
    test('download image add auth headers success case test', () async {
      await cacheManager
          .download(url: "www://test.com/image.png", headers: {"token": ""});
      verify(cacheManager
          .download(url: 'www://test.com/image.png', headers: {"token": ""}));
    });
    test('download image failed case test', () async {
      ...
      expect(() async => cacheManager.download(url: url),
    ...
    test('get cache from memory success test', () async {
      when(cacheManager.getFormMemoryCache("key"))
    ...
    test('get cache from file success test', () async {
      const url = "url";
      when(cacheManager.downloadStream(url: url))
